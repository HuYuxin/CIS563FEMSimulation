...
bool parseFile(std::string fileName, std::vector<particle>& particles, std::vector<std::pair<int, int>>& edges,
    ...
    while(std::getline(infile, line)){
        ...
        if(totalPoints > 0){
            ...
            float z;
            iss >> z;
            //Added By Yuxin to rotate Mario Model around Z axis by -90 degrees
            ...
            Eigen::Vector3d originPos(x, y, z);
            Eigen::Vector3d transPos = transform * transform2 * originPos;
            particle p(transPos[0], transPos[1], transPos[2]);*/
            particle p(x,y,z);
    ...
    for(int index = 0; index < tetrahedralssize; index++)
    {
        ...
        switch(forceModel) {
            ...
            case NEOHOOKEAN: {
                ...
                float J = F.determinant();
                P = miu*(F - miu*F.inverse().transpose()) + lamda*log(J)*F.inverse().transpose();
                //Todo, Need to replace F by the nearest physically plausible Fdelta where determinate of Fdelta is bigger than 0
                break;
            }
        }
...
int main(int argc, char* argv[]){
    ...
    std::vector<particle> particleNHKForce;
    ...
    for(unsigned int i = 0; i<particles.size(); i++){
        ...
        particleNHKForce.push_back(particle(particles[i].posx+48.0, particles[i].posy, particles[i].posz));
        particleNHKForce[i].mass = particles[i].mass;
    }
    ...
    int simulationTime = 6; //30; //Seconds
    int framesPerSecond = 24;
    int timeStepCountPerFrame = 600;
    ...
    for(int i = 0; i < simulationTime; i++)
    {
        for(int j = 0; j < framesPerSecond; j++)
        {
            for(int k = 0; k < timeStepCountPerFrame; k++)
            {
                ...
                //evaluateForce(tetrahedrals, particleLinearForce,SVKIRCHHOFF);
                //evaluateForce(tetrahedrals, particleNHKForce, NEOHOOKEAN);
                ...
                //updateParticles(particleLinearForce, simulationTimeStep);
                //updateParticles(particleNHKForce, simulationTimeStep);
                ...
                //kinematicCollision(particleLinearForce);
                //kinematicCollision(particleNHKForce);
            }

            //*************TODO: write the particle position and velocity in bgeo file***********//
            writeBgeoFile(particles, "simulationResult/cubeDebug/femSimulationCubeCorotated"+std::to_string(frameNum)+".bgeo");


            //if(!writePolyFile("simulationResult/Mario/femSimulationMarioCorotated" + std::to_string(frameNum)+ ".poly", particles, edges)){
                //std::cout<<"write poly file error!! check the .poly file"<<std::endl;
            //}
            ...
            //if(!writePolyFile("simulationResult/femSimulationNHK" + std::to_string(frameNum)+ ".poly", particleNHKForce, edges)){
                //std::cout<<"write poly file error!! check the .poly file"<<std::endl;
            //}

            //if(!writeObjFile("simulationResult/femsimulationCube" + std::to_string(frameNum)+ ".obj", particles, faces)){
